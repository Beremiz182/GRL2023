...
path = r"C:\Users\lyra001\OneDrive - Wageningen University & Research\articles_project\matlab\EartH2Observe\Cross\csv_to_RF_final/"
...
for MODEL in MODELS:
    ...
    for output_VAR in Indices:
        ...
        for MODEL2 in MODELS:
            ...
            x = path + MODEL + "_" + output_VAR + "_" + MODEL2 + ".csv"
            data = pd.read_csv(x)
            ...
            x2 = pd.DataFrame(FI)
            x2.to_csv('./OUTPUT_FILES_cross/FI_' + MODEL + '_' + output_VAR + '_' + MODEL2 + '.csv', sep=',')

            huffs = data[[output_VAR, "predict1"]]  # , "predict2", "predict3"
            huffs.to_csv('./OUTPUT_FILES_cross/result_all_' + MODEL + '_' + output_VAR + '_' + MODEL2 + '.csv', sep=',')
            huffs = data2[[output_VAR, "predict1"]]  # , "predict2", "predict3"
            huffs.to_csv('./OUTPUT_FILES_cross/result_train_' + MODEL + '_' + output_VAR + '_' + MODEL2 + '.csv', sep=',')
            huffs = data3[[output_VAR, "predict1"]]  # , "predict2", "predict3"
            huffs.to_csv('./OUTPUT_FILES_cross/result_test_' + MODEL + '_' + output_VAR + '_' + MODEL2 + '.csv', sep=',')
